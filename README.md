...
**Analysis:** Following the discovery of the initial payload domain, a pivot was conducted to identify related attacker infrastructure. A search within `DeviceNetworkEvents` for the root domain `cloud-endpoint.net` revealed a secondary domain. The logs showed later outbound connections to `cdn.cloud-endpoint.net`, identifying it as the staging domain for the ransomware payload.
...
**Analysis:** During the investigation of the initial payload domain (`sync.cloud-endpoint.net`), the network logs were reviewed to identify the specific IP addresses hosting the attacker's infrastructure. The malicious domain resolved to two distinct C2 IP addresses.
...
**Analysis:** To identify the infrastructure used for interactive remote access, network connections initiated by known remote access tools were reviewed. Querying `DeviceNetworkEvents` for `anydesk.exe` revealed outbound connections to the vendor's infrastructure. The specific relay domain used by the threat actor was identified as `relay-0b975d23.net.anydesk.com`.
